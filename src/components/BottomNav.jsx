...
export default function BottomNav({ activeTab, onChange }) {
  return (
    <nav className="fixed bottom-3 left-1/2 z-50 w-[calc(100%-1.25rem)] max-w-lg -translate-x-1/2">
          ...
          return (
            ...
            >
              ...
              <div className="relative flex flex-col items-center justify-center gap-0.5">
                <Ic className={`w-5 h-5 ${active ? 'text-ios-blue' : 'text-gray-400'}`} />
                <span className={`text-[10px] font-medium leading-none ${active ? 'text-ios-blue' : 'text-gray-500'}`}>
                  {label}
                </span>
              </div>
            </button>
