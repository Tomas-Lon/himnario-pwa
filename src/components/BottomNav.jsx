...
export default function BottomNav({ activeTab, onChange }) {
  return (
    <nav className="fixed bottom-3 left-1/2 z-50 w-[min(calc(100%-1rem),32rem)] sm:w-[min(calc(100%-1.5rem),30rem)] -translate-x-1/2">
      <div className="tab-bar rounded-[2rem] border border-black/5 bg-white/88 backdrop-blur-2xl shadow-[0_16px_36px_rgba(32,32,32,0.12)] px-2.5 py-2">
        ...
        {TABS.map(({ id, label, Icon, ActiveIcon }) => {
          ...
          return (
            <button
              key={id}
              onClick={() => onChange(id)}
              className="relative flex-1 min-w-0 h-14 sm:h-[3.75rem] flex items-center justify-center transition-transform active:scale-95"
            >
              {active && (
                <span className="absolute inset-x-2 top-1 bottom-1 rounded-[1.15rem] bg-[#f2f3f7] shadow-[inset_0_0_0_1px_rgba(17,24,39,0.04)]" />
              )}
              <div className="relative flex flex-col items-center justify-center gap-1 leading-none text-center">
                <Ic className={`w-4 h-4 sm:w-5 sm:h-5 shrink-0 ${active ? 'text-ios-blue' : 'text-gray-400'}`} />
                <span className={`text-[9px] sm:text-[10px] font-medium ${active ? 'text-ios-blue' : 'text-gray-500'}`}>
                  {label}
                </span>
              </div>
            </button>
